...
impl PdfLineItem {
    pub fn new(source_dir: &str, path: &str) -> Result<PdfLineItem, String> {
		...
		if cost_as_float.is_err() {
			return Err(format!("INVALID COST: PdfLineItem 'cost' failed to convert to a floating point number\n{}", trimmed_path).to_owned());
		}
    ...
    pub fn new_from_dir(source_dir: &str) -> Result<Vec<PdfLineItem>, String> {
        ...
        if !dir_path.exists() {
            return Err("MISSING FILE: provided an missing file path for run_sort()".to_owned());
        }
        if !dir_path.is_dir() {
            return Err("INVALID FILE: the provided file path must be a directory".to_owned());
        }
